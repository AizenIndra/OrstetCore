...
namespace AccountMgr
{
    ...
    time_t GetVipEndTime(uint32 accountId)
    {
    ...
    void SetVipStatus(uint32 accountId, time_t endTime)
    {
        LoginDatabasePreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SET_ACCOUNT_PREMIUM);
        stmt->SetData(0, accountId);
        stmt->SetData(1, uint64(endTime));
        stmt->SetData(2, true);     // active
        LoginDatabase.Execute(stmt);
    }
    ...
    void UpdateVipStatus(uint32 accountId, time_t endTime)
    {
        // Upsert VIP. StartTime is reset server-side to CURRENT_TIMESTAMP().
        LoginDatabasePreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_ACCOUNT_PREMIUM_NEW);
        stmt->SetData(0, accountId);
        stmt->SetData(1, uint64(endTime));
        stmt->SetData(2, uint64(endTime));
        LoginDatabase.Execute(stmt);
    }
