...
void TransmogrificationMgr::LoadFromDB()
{
    ...
    if (!result)
    {
        //TC_LOG_INFO("server.loading", ">> Loaded 0 transmogrified items. DB table `item_transmogrification` is empty!");
        return;
    }
    ...
    //TC_LOG_INFO("server.loading", ">> Loaded %u transmogrified items in %u ms", uint32(_transmogrificationStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
